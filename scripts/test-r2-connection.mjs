...
console.log("Bucket        :", R2_BUCKET_NAME || "MISSING");
...
try {
  ...
  await client.send(new PutObjectCommand({ Bucket: R2_BUCKET_NAME, Key: testKey, Body: testBody, ContentType: "text/plain" }));
  console.log("  ✓ upload OK");

  console.log("→ GET  ", testKey);
  const got = await client.send(new GetObjectCommand({ Bucket: R2_BUCKET_NAME, Key: testKey }));
  ...
  await client.send(new DeleteObjectCommand({ Bucket: R2_BUCKET_NAME, Key: testKey }));
